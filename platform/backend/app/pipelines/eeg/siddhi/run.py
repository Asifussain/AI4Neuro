...
if __name__ == '__main__':
    ...
    # Where to write prediction results. Defaults to 'output.json' in CWD to
    # preserve the original behaviour; the unified pipeline passes a UNIQUE path
    # per job so concurrent runs never collide.
    parser.add_argument('--output_path', type=str, default='output.json',
                        help='Path to write the prediction results JSON')
    # Optional override for the checkpoints root. When set, checkpoints are
    # resolved under <checkpoint_root>/<task>/<model_id>/<model>/<setting>/. When
    # unset, the original __file__-relative '../checkpoints' path is used.
    ...
    parser.add_argument('--seq_len', type=int, default=128, help='input sequence length (used for setting string & model)')
    ...
    exp.predict_unlabeled_sample(
        args.input_file, setting, device,
