...
def test_report_generation_matrix_for_all_analysis_variants(
    ...
    # clinician/technical share one unified PDF; patient gets its own
    # dedicated plain-language PDF — two distinct stored files.
    assert len(stored) == 2
    assert reports.clinician_pdf_url == reports.technical_pdf_url
    assert reports.patient_pdf_url != reports.clinician_pdf_url
