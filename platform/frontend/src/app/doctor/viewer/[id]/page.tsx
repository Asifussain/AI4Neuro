...
const predictionConfig = {
  ...
  MCI: {
    ...
    description: 'Early signs of cognitive decline detected.',
  },
  AD: {
      ...
      <div className="relative z-10 max-w-[1920px] mx-auto space-y-4 py-2">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl lg:text-2xl font-bold text-foreground flex items-center gap-2">
                <Brain className="h-5 w-5 lg:h-6 lg:w-6 text-purple-500" />
                <GradientText>Clinical Review</GradientText>
              </h1>
          ...
          <div className="flex flex-wrap gap-2">
            {(prediction?.clinician_pdf_url || prediction?.patient_pdf_url || prediction?.technical_pdf_url) && (
              <Button variant="outline" size="sm" asChild className="text-xs">
                <a
                  href={(prediction.clinician_pdf_url || prediction.patient_pdf_url || prediction.technical_pdf_url) ?? undefined}
                  target="_blank"
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-4">
                ...
                {prediction?.prediction ? (
                  <div className="space-y-2">
                    {prediction.prediction === 'AD' && (
                      <>
                        <RecommendationItem text="Comprehensive cognitive assessment recommended" />
                        <RecommendationItem text="Consider pharmacological intervention (cholinesterase inhibitors)" />
                        <RecommendationItem text="Follow-up MRI in 6 months to monitor progression" />
                        <RecommendationItem text="Refer to memory clinic for specialized care" />
                        <RecommendationItem text="Discuss care planning with family" />
                      </>
                    )}
                    {prediction.prediction === 'MCI' && (
                      <>
                        <RecommendationItem text="Cognitive screening and monitoring recommended" />
                        <RecommendationItem text="Lifestyle modifications (exercise, diet, cognitive activities)" />
                        <RecommendationItem text="Monitor for progression to dementia" />
                        <RecommendationItem text="Follow-up MRI in 12 months" />
                        <RecommendationItem text="Consider neuropsychological testing" />
                      </>
                    )}
                    {prediction.prediction === 'CN' && (
                      <>
                        <RecommendationItem text="Routine follow-up as scheduled" />
                        <RecommendationItem text="Maintain healthy lifestyle habits" />
                        <RecommendationItem text="No immediate intervention required" />
                        <RecommendationItem text="Continue age-appropriate health screenings" />
                      </>
                    )}
                  </div>
                ) : (
          ...
          <div className="xl:col-span-3 space-y-4">
                ...
                <div className="p-4">
                  ...
                  <div className="space-y-3">
                    {prediction.brain_volume != null && prediction.brain_volume > 0 && (
                      <VolumeBar label="Total Brain" value={prediction.brain_volume} normMin={1100} normMax={1400} unit="cm³" />
                    )}
                    {prediction.gm_volume != null && prediction.gm_volume > 0 && (
                      <VolumeBar label="Gray Matter" value={prediction.gm_volume} normMin={450} normMax={600} unit="cm³" />
                    )}
                    {prediction.wm_volume != null && prediction.wm_volume > 0 && (
                      <VolumeBar label="White Matter" value={prediction.wm_volume} normMin={400} normMax={550} unit="cm³" />
                    )}
                    {prediction.csf_volume != null && prediction.csf_volume > 0 && (
                      <VolumeBar label="CSF" value={prediction.csf_volume} normMin={150} normMax={300} unit="cm³" />
                    )}
                    {prediction.hippocampal_volume != null && prediction.hippocampal_volume > 0 && (
                      <VolumeBar label="Hippocampus" value={prediction.hippocampal_volume} normMin={3.0} normMax={4.5} unit="cm³" />
                    )}
                    {prediction.ventricular_volume != null && prediction.ventricular_volume > 0 && (
                      <VolumeBar label="Ventricles" value={prediction.ventricular_volume} normMin={20} normMax={50} unit="cm³" invertWarning />
                    )}
                  </div>
            ...
            {/* Report */}
            {prediction && (prediction.clinician_pdf_url || prediction.patient_pdf_url || prediction.technical_pdf_url) && (
              <SpotlightCard spotlightColor="rgba(147, 51, 234, 0.06)">
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-3">
                  ...
                  <div className="space-y-1.5">
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-2 text-xs text-muted-foreground hover:text-purple-400 transition-colors p-1.5 rounded-md hover:bg-white/5 group"
                    >
...
function RecommendationItem({ text }: { text: string }) {
  return (
    <div className="flex items-start gap-2 text-xs text-muted-foreground">
      <ChevronRight className="h-3 w-3 mt-0.5 text-purple-500/70 shrink-0" />
